...
- **Snakemake utilities** — callbacks and CLI helpers for Snakemake-based training pipelines
- **Metrics** — `RejectionAtFixedEfficiency` (background rejection at a target signal efficiency)
...
PairFormer processes both a single (per-token) representation and a pair (token-token) representation jointly, as used in [AlphaFold 2](https://www.nature.com/articles/s41586-021-03819-2).
...
`WandBSaveConfigCallback` uploads the Lightning CLI config file as a WandB artifact when using `WandbLogger`.

---

### Snakemake utilities (`ml_utils.snakemake`)

Helpers for building command-line strings for the [Lightning CLI](https://lightning.ai/docs/pytorch/stable/cli/lightning_cli.html) inside Snakemake workflows.
...
cmd = " ".join([
    ...
    add_checkpoint_command(
        dir_path=Path("checkpoints"),
        monitor="val/loss",
        mode="min",
        save_top_k=3,
    ),
    add_early_stopping_command(monitor="val/loss", patience=10),
    add_snakemake_confirm_callback_command(output_file="done.txt"),
])
```

`SnakemakeConfirmFinish` is a Lightning callback that writes a completion file when training finishes (required by Snakemake to mark an output as produced).

---

### Metrics (`ml_utils.metrics`)

#### RejectionAtFixedEfficiency

Computes background rejection (1 / background efficiency) at a fixed signal efficiency. Useful for high-energy physics classification tasks.

```python
from ml_utils.metrics import RejectionAtFixedEfficiency

metric = RejectionAtFixedEfficiency(
    signal_indices=0,          # class index(es) treated as signal
    background_indices=[1, 2], # class index(es) treated as background
    target_efficiency=0.9,     # keep 90% of signal
    from_logits=True,
)

metric.update(logits, targets)
rejection = metric.compute()  # 1 / background_efficiency
```
